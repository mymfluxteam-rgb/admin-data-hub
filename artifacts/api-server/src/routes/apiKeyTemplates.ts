...
router.get("/:id", async (req, res) => {
  const { data, error } = await supabase
    ...
    .eq("id", req.params["id"])
    .single();
  if (error) { res.status(404).json({ message: "Template not found" }); return; }
  res.json(data);
});

// POST /api/api-key-templates — create a template
router.post("/", async (req, res) => {
  const { template_name, permissions, rate_limit, expires_in_days, description } = req.body as {
...
router.put("/:id", async (req, res) => {
  ...
  if (template_name !== undefined) updates["template_name"] = template_name.trim();
  if (permissions !== undefined) updates["permissions"] = permissions;
  if (rate_limit !== undefined) updates["rate_limit"] = rate_limit;
  if (expires_in_days !== undefined) updates["expires_in_days"] = expires_in_days;
