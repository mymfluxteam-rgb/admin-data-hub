...
router.put("/:id/credits/add", async (req, res) => {
  ...
  if (fetchErr || !user) { res.status(404).json({ message: fetchErr?.message ?? "User not found" }); return; }

  const newCredits = (user["credits"] as number) + amount;
  const { data, error } = await supabase
...
router.put("/:id/credits/remove", async (req, res) => {
  ...
  const newCredits = Math.max(0, (user["credits"] as number) - amount);
  const { data, error } = await supabase
    .from("users").update({ credits: newCredits }).eq("id", req.params["id"]).select().single();
  if (error) { res.status(500).json({ message: error.message }); return; }
...
router.post("/:id/set-expiry", async (req, res) => {
  const { years = 0, months = 0, days = 0, hours = 0, minutes = 0, seconds = 0, manualDate } = req.body as {
...
router.post("/:id/extend-expiry", async (req, res) => {
  const { days = 0, hours = 0 } = req.body as { days?: number; hours?: number };

  const { data: user, error: fetchErr } = await supabase
    .from("users").select("account_expiry").eq("id", req.params["id"]).single();
  if (fetchErr || !user) { res.status(404).json({ message: "User not found" }); return; }

  const base = user["account_expiry"] ? new Date(user["account_expiry"] as string) : new Date();
...
router.delete("/:id", async (req, res) => {
  ...
  await supabase.from("audit_logs").insert({
    action: "user.delete",
    actor: "admin",
    target: (user as Record<string, unknown>)["email"] as string,
    details: `Deleted user: ${(user as Record<string, unknown>)["username"]}`,
  });
  res.status(204).send();
});
