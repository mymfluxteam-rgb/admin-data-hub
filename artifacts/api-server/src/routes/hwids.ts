...
router.get("/", async (_req, res) => {
  ...
  const rows = (data ?? []).map((h: Record<string, unknown>) => ({
    ...h,
    user_email: (h["users"] as Record<string, unknown> | null)?.["email"] ?? null,
    users: undefined,
  }));
  res.json(rows);
});
...
router.put("/:id/blacklist", async (req, res) => {
  const { blacklisted } = req.body as { blacklisted?: boolean };
  if (typeof blacklisted !== "boolean") {
    res.status(400).json({ message: "blacklisted must be a boolean" }); return;
  }
  const { data, error } = await supabase
    .from("hwids").update({ blacklisted }).eq("id", req.params["id"]).select().single();
  if (error) { res.status(500).json({ message: error.message }); return; }

  await supabase.from("audit_logs").insert({
    action: blacklisted ? "hwid.blacklist" : "hwid.unblacklist",
    actor: "admin", target: req.params["id"],
...
router.put("/:id/restrict", async (req, res) => {
  const { hours = 0, minutes = 0, seconds = 0 } = req.body as {
    hours?: number; minutes?: number; seconds?: number;
  };
  const totalMs = ((hours * 3600) + (minutes * 60) + seconds) * 1000;
  if (totalMs <= 0) {
    res.status(400).json({ message: "Restriction duration must be greater than 0" }); return;
  }
  ...
  await supabase.from("audit_logs").insert({
    action: "hwid.restrict", actor: "admin", target: req.params["id"],
...
router.put("/:id/manual-block", async (req, res) => {
  ...
  await supabase.from("audit_logs").insert({
    action: "hwid.manual_block", actor: "admin", target: req.params["id"],
...
router.put("/:id/unblock", async (req, res) => {
  ...
  await supabase.from("audit_logs").insert({
    action: "hwid.unblock", actor: "admin", target: req.params["id"],
...
router.delete("/:id", async (req, res) => {
  const { error } = await supabase.from("hwids").delete().eq("id", req.params["id"]);
  if (error) { res.status(500).json({ message: error.message }); return; }
  res.status(204).send();
});
