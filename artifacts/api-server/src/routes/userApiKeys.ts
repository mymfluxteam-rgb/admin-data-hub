...
router.post("/", async (req, res) => {
  const { app_name, user_id } = req.body as {
    app_name?: string;
    user_id?: string;
  };
...
router.post("/:id/revoke", async (req, res) => {
  ...
  await supabase.from("audit_logs").insert({
    ...
    details: `Revoked API key for app "${(existing as Record<string, unknown>)["app_name"]}"`,
  }).then(() => {});
...
router.delete("/:id", async (req, res) => {
  ...
  await supabase.from("audit_logs").insert({
    ...
    actor: "admin",
    target: req.params["id"],
    details: `Deleted API key for app "${(existing as Record<string, unknown>)["app_name"]}"`,
  }).then(() => {});
