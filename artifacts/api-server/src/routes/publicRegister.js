...
function extractCredentials(req) {
    const publicKey = req.headers["x-api-key"]?.trim() ||
        req.body["public_key"]?.trim() ||
        null;
    const appSecret = req.headers["x-api-secret"]?.trim() ||
...
router.post("/register", registrationLimiter, async (req, res) => {
    ...
    if (!publicKey || !appSecret) {
        res.status(401).json({
            ...
            message: "Provide your Public API Key via the x-api-key header (or body.public_key) " +
    ...
    if (keyErr) {
        if (keyErr.code === "42P01") {
            ...
            res.status(503).json({
                ...
                message: "The user_api_keys table does not exist. " +
    ...
    const { username, email, password, hwid, role } = req.body;
    const errors = [];
    ...
    const { data: newUser, error: insertErr } = await supabase
        .from("users")
        .insert(userPayload)
        .select("id, username, email, status, verified, credits, role, public_api_key, account_expiry, owner_id, registered_via_key_id, created_at")
        .single();
    ...
    if (insertErr) {
        if (insertErr.code === "42703") {
            ...
            const { username: _un, email: _em, ...restPayload } = userPayload;
    ...
    await supabase.from("audit_logs").insert({
        ...
        details: `Public API registration — app: "${keyRow.app_name}", ` +
