...
// ── PKCE in-memory store (code verifier keyed by opaque state token) ──────────
interface PkceEntry {
  codeVerifier: string;
  expiresAt: number;
}
...
function buildOAuthUrl(provider: "google" | "github", req: Request): { url: string; state: string } {
  cleanupPkce();
...
router.get("/callback", async (req, res) => {
  ...
  const tokenData = (await tokenRes.json()) as {
    access_token?: string;
    user?: { email?: string; identities?: { provider: string }[] };
    error?: string;
