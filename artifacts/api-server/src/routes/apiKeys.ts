...
router.get("/:id", async (req, res) => {
  const { data, error } = await supabase
    ...
    .eq("id", req.params["id"])
    .single();
  if (error) { res.status(404).json({ message: "API key not found" }); return; }
  res.json(data);
});
...
router.post("/", async (req, res) => {
  ...
  if (template_id) {
    ...
    resolvedPermissions = (tmpl as Record<string, unknown>)["permissions"] as string[];
    resolvedRateLimit = (tmpl as Record<string, unknown>)["rate_limit"] as number ?? rate_limit;
    if (!resolvedExpiresAt && (tmpl as Record<string, unknown>)["expires_in_days"]) {
      const d = new Date();
      d.setDate(d.getDate() + ((tmpl as Record<string, unknown>)["expires_in_days"] as number));
  ...
  await supabase.from("audit_logs").insert({
    ...
    target: (data as Record<string, unknown>)["id"] as string,
...
router.post("/:id/revoke", async (req, res) => {
  const { revoked_by, revoke_reason } = req.body as {
    revoked_by?: string;
...
router.post("/:id/rotate", async (req, res) => {
  const { rotated_by } = req.body as { rotated_by?: string };
  ...
  await supabase.from("audit_logs").insert({
    ...
    target: req.params["id"],
    details: `Rotated API key "${(existing as Record<string, unknown>)["key_name"]}"`,
  });
...
router.delete("/:id", async (req, res) => {
  ...
  await supabase.from("audit_logs").insert({
    ...
    actor: "admin",
    target: req.params["id"],
    details: `Deleted API key "${(existing as Record<string, unknown>)["key_name"]}"`,
  });
...
router.post("/validate", async (req, res) => {
  const { api_key, ip_address, endpoint, method } = req.body as {
    api_key?: string;
    ip_address?: string;
    endpoint?: string;
    method?: string;
  ...
  const allowedIps = key["allowed_ips"] as string[];
  ...
  await supabase
    .from("api_keys")
    .update({
      usage_count: (key["usage_count"] as number) + 1,
...
router.post("/:id/usage", async (req, res) => {
  const { user_id, endpoint, method, ip_address, user_agent, response_status, response_time_ms } = req.body as {
    user_id?: string;
    endpoint?: string;
    method?: string;
    ip_address?: string;
