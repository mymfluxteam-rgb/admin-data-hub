...
import { Globe, Users, Key, Cpu, CreditCard, ShieldCheck, Bell, Activity, HardDrive, UserCog, } from "lucide-react";
...
export default function SettingsPage() {
    ...
    return (<div className="space-y-4">
      ...
      <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v)} className="flex gap-6 min-h-[calc(100vh-12rem)] items-start" orientation="vertical">
        <TabsList className="flex flex-col h-auto w-52 shrink-0 bg-sidebar border border-border/50 rounded-lg p-1.5 gap-0.5 sticky top-0">
          {TAB_DEFS.map(({ value, key, icon: Icon }) => (<TabsTrigger key={value} value={value} className="w-full justify-start gap-2.5 px-3 py-2 text-sm rounded-md data-[state=active]:bg-primary/10 data-[state=active]:text-primary data-[state=active]:font-medium data-[state=inactive]:text-muted-foreground">
