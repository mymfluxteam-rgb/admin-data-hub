...
import { Trash2, ShieldBan, Timer, ShieldX, ShieldCheck, Monitor, Smartphone, Mail, AlertTriangle, } from "lucide-react";
...
function RestrictionBadge({ h }) {
    if (!h.restriction_type)
        return null;
    ...
    if (h.restriction_type === "time_limit" && h.restriction_end_time) {
        ...
        if (remaining <= 0)
            return <span className="text-xs text-muted-foreground block">Restriction expired</span>;
...
function EmailConfirmDialog({ action, onConfirm, onCancel, }) {
    const labels = {
    ...
    return (<Dialog open onOpenChange={(o) => !o && onCancel()}>
      <DialogContent className="sm:max-w-sm">
        ...
        <DialogFooter>
          ...
          <Button size="sm" className={action.type === "delete" || action.type === "manual-block" ? "bg-destructive hover:bg-destructive/90" : ""} onClick={onConfirm}>
            Confirm
          </Button>
        </DialogFooter>
      </DialogContent>
...
export default function HWIDsPage() {
    ...
    const executeAction = async () => {
        ...
        if (type === "blacklist")
            ok = await hwidsApi.setBlacklist(hwid.id, true);
        if (type === "unblacklist")
            ok = await hwidsApi.setBlacklist(hwid.id, false);
        if (type === "manual-block")
            ok = await hwidsApi.manualBlock(hwid.id);
        if (type === "unblock")
            ok = await hwidsApi.unblock(hwid.id);
        if (ok) {
            const msgs = {
    ...
    const columns = [
        {
            ...
            render: (h) => (<div className="flex items-center gap-1.5">
        ...
        },
        {
            ...
            render: (h) => (<div className="text-xs text-muted-foreground space-y-0.5">
        ...
        },
        {
            ...
            render: (h) => (<div className="space-y-0.5">
          <StatusBadge status={h.blacklisted ? "blacklisted" : "active"}/>
          <RestrictionBadge h={h}/>
        </div>),
        },
        { key: "last_seen", label: "Last Seen", render: (h) => <span className="text-xs text-muted-foreground">{new Date(h.last_seen).toLocaleDateString()}</span> },
        {
            key: "actions", label: "Actions",
            render: (h) => (<div className="flex gap-1 flex-wrap">
          <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-destructive" title="Delete device" onClick={() => confirmAction(h, "delete")}>
            <Trash2 className="h-3.5 w-3.5"/>
          </Button>
          <Button variant="ghost" size="icon" className={`h-7 w-7 text-muted-foreground ${h.blacklisted ? "hover:text-emerald-400" : "hover:text-yellow-400"}`} title={h.blacklisted ? "Remove blacklist" : "Blacklist"} onClick={() => confirmAction(h, h.blacklisted ? "unblacklist" : "blacklist")}>
            <ShieldBan className="h-3.5 w-3.5"/>
          </Button>
          <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-orange-400" title="Set login time restriction (H:M)" onClick={() => { setRestrictTarget(h); setRHours("1"); setRMinutes("0"); }}>
            <Timer className="h-3.5 w-3.5"/>
          </Button>
          {h.restriction_type ? (<Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-emerald-400" title="Remove all restrictions" onClick={() => confirmAction(h, "unblock")}>
              <ShieldCheck className="h-3.5 w-3.5"/>
            </Button>) : (<Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-red-400" title="Manual block" onClick={() => confirmAction(h, "manual-block")}>
    ...
    return (<div className="space-y-6">
      ...
      <DataTable data={hwids} columns={columns} searchKey="device_id" searchPlaceholder="Search by device ID..." isLoading={loading}/>

      {pendingAction && (<EmailConfirmDialog action={pendingAction} onConfirm={executeAction} onCancel={() => setPendingAction(null)}/>)}
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Input type="number" min={0} max={99} value={rHours} onChange={(e) => setRHours(e.target.value)} className="mt-1 font-mono"/>
              </div>
              <div>
                <Label className="text-xs">Minutes</Label>
                <Input type="number" min={0} max={59} value={rMinutes} onChange={(e) => setRMinutes(e.target.value)} className="mt-1 font-mono"/>
              </div>
            </div>

            {(parseInt(rHours) > 0 || parseInt(rMinutes) > 0) && (<p className="text-xs text-muted-foreground">
                Login blocked until:{" "}
