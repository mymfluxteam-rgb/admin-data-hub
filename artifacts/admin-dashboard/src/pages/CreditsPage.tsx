...
import { usersApi, transactionsApi, creditsApi, type User, type Transaction } from "@/lib/api";
...
export default function CreditsPage() {
  ...
  const fetchAll = () => {
    setLoading(true);
    Promise.all([usersApi.getAll(), transactionsApi.getAll()]).then(
      ([u, t]) => { setUsers(u); setTransactions(t); }
    ).finally(() => setLoading(false));
  };

  useEffect(() => { fetchAll(); }, []);

  const openAdjust = (user: User, mode: "add" | "remove") => {
  ...
  const balanceColumns = [
    { key: "username", label: "User", render: (u: User) => <span className="font-mono text-sm">{u.username}</span> },
    {
    ...
    },
    {
      ...
      render: (u: User) => <span className="font-mono font-medium">{(u.credits ?? 0).toLocaleString()}</span>,
    },
      ...
      render: (u: User) => (
        <div className="flex gap-1">
          <Button variant="ghost" size="sm"
            className="h-7 text-xs text-muted-foreground hover:text-emerald-400 hover:bg-emerald-400/10 gap-1 px-2"
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <DataTable
          ...
          columns={balanceColumns as Parameters<typeof DataTable>[0]["columns"]}
          searchKey="email" searchPlaceholder="Search by email..." isLoading={loading}
        />
      </div>
      ...
      <div>
        ...
        <DataTable
          data={transactions.slice(0, 20) as unknown as Record<string, unknown>[]}
        ...
        <DialogContent className="sm:max-w-sm">
          ...
          <div className="space-y-3">
            <div>
              ...
              <Input
                ...
                placeholder="e.g. 500"
              />
              {adjustTarget && !isNaN(parseInt(adjustAmount)) && parseInt(adjustAmount) > 0 && (
                <p className="text-xs text-muted-foreground mt-1">
                  New balance:{" "}
          ...
          <DialogFooter>
            ...
            <Button
              size="sm"
              disabled={adjusting}
              className={adjustTarget?.mode === "remove" ? "bg-destructive hover:bg-destructive/90" : ""}
