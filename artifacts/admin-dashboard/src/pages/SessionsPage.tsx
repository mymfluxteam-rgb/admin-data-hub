...
import { sessionsApi, type DeviceSession } from "@/lib/api";
...
function DeviceIcon({ type }: { type?: string | null }) {
  if (type === "mobile") return <Smartphone className="h-4 w-4 text-blue-400" />;
  if (type === "chrome") return <Chrome className="h-4 w-4 text-yellow-400" />;
  return <Monitor className="h-4 w-4 text-muted-foreground" />;
}

function TimeAgo({ date }: { date: string }) {
  const diff = Date.now() - new Date(date).getTime();
  if (diff < 60000) return <span className="text-xs text-emerald-400">Just now</span>;
  if (diff < 3600000) return <span className="text-xs text-emerald-400">{Math.floor(diff / 60000)}m ago</span>;
  if (diff < 86400000) return <span className="text-xs text-yellow-400">{Math.floor(diff / 3600000)}h ago</span>;
...
export default function SessionsPage() {
  ...
  const handleForceLogout = async (s: DeviceSession) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (s: DeviceSession) => (
        <div className="text-xs text-muted-foreground">{new Date(s.login_time).toLocaleString()}</div>
      ),
    },
    {
      key: "last_activity", label: "Last Active",
      render: (s: DeviceSession) => <TimeAgo date={s.last_activity} />,
    },
    {
      key: "actions", label: "Actions",
      render: (s: DeviceSession) => (
        <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-destructive"
          title="Force logout" onClick={() => handleForceLogout(s)}>
