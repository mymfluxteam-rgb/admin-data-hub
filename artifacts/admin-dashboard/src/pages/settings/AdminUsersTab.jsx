...
function RoleBadge({ role }) {
    ...
    return (<span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-medium ${found.color}`}>
      {found.label}
    </span>);
}
...
export default function AdminUsersTab() {
    ...
    const handleAdd = async () => {
        ...
        const found = all.find((u) => u.email.toLowerCase() === addEmail.trim().toLowerCase());
    ...
    return (<div className="space-y-6">
      <Card className="border-border/50">
        ...
        <CardContent className="space-y-3">
          <div className="relative">
            ...
            <Input placeholder="Search by email or username..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} className="pl-9"/>
          </div>

          {loading ? (<div className="space-y-2">
              {Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} className="h-12 w-full"/>)}
            </div>) : filtered.length === 0 ? (<div className="text-center py-8 text-muted-foreground text-sm">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {paged.map((admin) => (<tr key={admin.id} className="border-b border-border/20 last:border-0 hover:bg-muted/30 transition-colors">
                            ...
                            <AlertDialog>
                              ...
                              <AlertDialogContent>
                                ...
                                <AlertDialogFooter>
                                  ...
                                  <AlertDialogAction onClick={() => handleDemote(admin.id, admin.email)} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">
                                    Remove
                                  </AlertDialogAction>
                                </AlertDialogFooter>
                              </AlertDialogContent>
              ...
              {totalPages > 1 && (<div className="flex items-center justify-between pt-2 text-xs text-muted-foreground">
        ...
        <DialogContent>
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-2">
              ...
              <Input id="add-email" type="email" value={addEmail} onChange={(e) => setAddEmail(e.target.value)} placeholder="user@example.com" onKeyDown={(e) => e.key === "Enter" && handleAdd()}/>
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent>
          ...
          {editTarget && (<div className="space-y-4 py-2">
              <div>
