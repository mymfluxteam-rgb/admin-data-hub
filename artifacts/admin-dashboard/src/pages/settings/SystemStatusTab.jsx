...
import { RefreshCw, Database, Server, Users, Key, AlertTriangle } from "lucide-react";
...
export default function SystemStatusTab() {
    const [statuses, setStatuses] = useState(ENDPOINTS.map((e) => ({ ...e, ok: null, status: null, latency: null })));
    const [metrics, setMetrics] = useState(null);
    const [activeApiKeys, setActiveApiKeys] = useState(null);
    ...
    const runChecks = async () => {
        ...
        setActiveApiKeys(keyList.filter((k) => k["is_active"]).length);
        const logList = Array.isArray(auditLogs) ? auditLogs : [];
        const yesterday = new Date(Date.now() - 24 * 60 * 60 * 1000);
        setRecentErrors(logList.filter((l) => {
    ...
    return (<div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          {lastChecked && (<p className="text-xs text-muted-foreground">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        ].map(({ label, value, icon: Icon, color }) => (<Card key={label} className="border-border/50">
            <CardContent className="p-4">
              <div className="flex items-center gap-2 mb-1">
                <Icon className={`h-4 w-4 ${color}`}/>
                <span className="text-xs text-muted-foreground">{label}</span>
              </div>
              {value !== null ? (<p className="text-2xl font-bold">{value}</p>) : (<Skeleton className="h-8 w-16"/>)}
            </CardContent>
          </Card>))}
      </div>
      ...
      <Card className="border-border/50">
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            ...
            {checking ? null : (<Badge variant={allOk ? "default" : "destructive"} className="text-xs">
                {allOk ? "All Healthy" : "Issues Detected"}
              </Badge>)}
          </div>
        </CardHeader>
        <CardContent className="space-y-0">
          {statuses.map((s) => (<div key={s.path} className="flex items-center justify-between py-2.5 border-b border-border/30 last:border-0">
              ...
              <div className="flex items-center gap-3 text-xs">
                {s.status !== null && (<span className={s.ok ? "text-emerald-400" : "text-red-400"}>{s.status}</span>)}
