      ...
      <Card className="border-border/50">
        ...
        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-2">
            ...
            <Input id="base_credit_price" type="number" min={0.01} step={0.01} value={settings.base_credit_price} onChange={(e) => update("base_credit_price", parseFloat(e.target.value) || 1.0)}/>
          </div>
          <div className="space-y-2">
            <Label htmlFor="free_credits_on_registration">{t("credits.freeOnReg")}</Label>
            <Input id="free_credits_on_registration" type="number" min={0} value={settings.free_credits_on_registration} onChange={(e) => update("free_credits_on_registration", parseInt(e.target.value) || 0)}/>
          </div>
          <div className="space-y-2">
            <Label htmlFor="min_credit_purchase">{t("credits.minPurchase")}</Label>
            <Input id="min_credit_purchase" type="number" min={1} value={settings.min_credit_purchase} onChange={(e) => update("min_credit_purchase", parseInt(e.target.value) || 10)}/>
          </div>
          <div className="space-y-2">
            <Label htmlFor="max_credit_purchase">{t("credits.maxPurchase")}</Label>
            <Input id="max_credit_purchase" type="number" min={1} value={settings.max_credit_purchase} onChange={(e) => update("max_credit_purchase", parseInt(e.target.value) || 10000)}/>
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="border-border/50">
        ...
        <CardContent className="space-y-4">
          <div className="max-w-xs space-y-2">
            ...
            <Input id="credit_expiry_days" type="number" min={0} value={settings.credit_expiry_days} onChange={(e) => update("credit_expiry_days", parseInt(e.target.value) || 0)}/>
          ...
          <div className="flex items-center justify-between py-2 border-t border-border/30 pt-4">
            ...
            <Switch checked={settings.auto_deduction_enabled} onCheckedChange={(v) => update("auto_deduction_enabled", v)}/>
          </div>
        </CardContent>
      </Card>
