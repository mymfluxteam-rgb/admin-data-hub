...
const BACKUP_DEFAULTS = { scheduled_backup: "daily", last_backup_time: null as string | null };

type ExportFormat = "json" | "csv";
type ExportType = "users" | "transactions" | "api_keys" | "audit_logs";
    ...
    <div className="space-y-6">
      <Card className="border-border/50">
        ...
        <CardContent className="space-y-4">
          {(settings.last_backup_time as string | null) && (
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              <Clock className="h-3.5 w-3.5" />
              Last backup: {new Date(settings.last_backup_time as string).toLocaleString()}
            </div>
          )}
      ...
      <Card className="border-border/50">
        ...
        <CardContent className="flex items-end gap-4">
          <div className="space-y-2">
            ...
            <Select
              value={settings.scheduled_backup as string}
              onValueChange={(v) => update("scheduled_backup", v)}
            >
      ...
      <Card className="border-border/50">
        ...
        <CardContent className="space-y-3">
          {EXPORT_ITEMS.map(({ type, label, description }) => (
