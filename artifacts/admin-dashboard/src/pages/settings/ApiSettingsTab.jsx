...
export default function ApiSettingsTab() {
    ...
    const removeOrigin = (origin) => {
        update("allowed_origins", settings.allowed_origins.filter((o) => o !== origin));
    };
    if (loading) {
      ...
      <Card className="border-border/50">
        ...
        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-2">
            ...
            <Input id="default_rate_limit" type="number" min={1} value={settings.default_rate_limit} onChange={(e) => update("default_rate_limit", parseInt(e.target.value) || 100)}/>
          </div>
          <div className="space-y-2">
            <Label htmlFor="default_key_expiry_days">{t("api.keyExpiry")}</Label>
            <Input id="default_key_expiry_days" type="number" min={1} value={settings.default_key_expiry_days} onChange={(e) => update("default_key_expiry_days", parseInt(e.target.value) || 365)}/>
            <p className="text-xs text-muted-foreground">{t("api.setToZero")}</p>
          </div>
          <div className="space-y-2">
            <Label htmlFor="api_version">{t("api.apiVersion")}</Label>
            <Select value={settings.api_version} onValueChange={(v) => update("api_version", v)}>
      ...
      <Card className="border-border/50">
        ...
        <CardContent className="space-y-3">
          <div className="flex gap-2">
            <Input value={newOrigin} onChange={(e) => setNewOrigin(e.target.value)} placeholder="https://app.example.com" onKeyDown={(e) => e.key === "Enter" && addOrigin()}/>
            <Button variant="outline" size="sm" onClick={addOrigin} className="shrink-0 gap-1">
              <Plus className="h-4 w-4"/> {t("api.add")}
            </Button>
          </div>
          <div className="flex flex-wrap gap-2 min-h-[2rem]">
            {settings.allowed_origins.length === 0 ? (<p className="text-xs text-muted-foreground">{t("api.corsEmpty")}</p>) : (settings.allowed_origins.map((origin) => (<Badge key={origin} variant="secondary" className="gap-1 font-mono text-xs">
                  {origin}
      ...
      <Card className="border-border/50">
        ...
        <CardContent className="space-y-3">
          <div className="flex items-center justify-between py-2 border-b border-border/30">
            ...
            <Switch checked={settings.api_access_enabled} onCheckedChange={(v) => update("api_access_enabled", v)}/>
          </div>
          <div className="flex items-center justify-between py-2">
            ...
            <Switch checked={settings.require_ip_whitelist} onCheckedChange={(v) => update("require_ip_whitelist", v)}/>
          </div>
        </CardContent>
      </Card>
