      ...
      <Card className="border-border/50">
        ...
        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-2">
            ...
            <Input id="system_name" value={settings.system_name} onChange={(e) => update("system_name", e.target.value)} placeholder="LicenseAdmin"/>
          </div>
          <div className="space-y-2">
            <Label htmlFor="logo_url">{t("general.logoUrl")}</Label>
            <Input id="logo_url" value={settings.logo_url} onChange={(e) => update("logo_url", e.target.value)} placeholder="https://..."/>
          </div>
          <div className="space-y-2">
      ...
      <Card className="border-border/50">
        ...
        <CardContent>
          <div className="space-y-2 max-w-sm">
            ...
            <Input id="contact_email" type="email" value={settings.contact_email} onChange={(e) => update("contact_email", e.target.value)} placeholder="admin@yourdomain.com"/>
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="border-amber-500/20 bg-amber-500/5">
        ...
        <CardContent>
          <div className="flex items-center gap-3">
            <Switch checked={settings.maintenance_mode} onCheckedChange={(v) => update("maintenance_mode", v)} className="data-[state=checked]:bg-amber-500"/>
            <span className="text-sm">
              {settings.maintenance_mode ? (<span className="text-amber-400 font-medium">{t("general.maintenanceOn")}</span>) : (<span className="text-muted-foreground">{t("general.maintenanceOff")}</span>)}
            </span>
          </div>
        </CardContent>
      </Card>
