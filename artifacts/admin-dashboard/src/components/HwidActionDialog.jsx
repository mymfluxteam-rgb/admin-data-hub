        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <Button type="button" variant={action === "blacklist" ? "default" : "outline"} size="sm" onClick={() => setAction("blacklist")}>Blacklist</Button>
              <Button type="button" variant={action === "delete" ? "destructive" : "outline"} size="sm" onClick={() => setAction("delete")}>Delete</Button>
            </div>
          </div>
          <div className="space-y-2">
            <Label htmlFor="hwidId">HWID Row ID (UUID)</Label>
            <Input id="hwidId" value={hwidId} onChange={(e) => setHwidId(e.target.value)} placeholder="xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx" required/>
          </div>
          <DialogFooter>
            <Button type="button" variant="ghost" onClick={() => setOpen(false)}>Cancel</Button>
            <Button type="submit" disabled={loading} variant={action === "delete" ? "destructive" : "default"}>
