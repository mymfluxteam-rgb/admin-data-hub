...
function CodeBlock({ code, language = "json" }) {
    ...
    return (<div className="relative group rounded-lg overflow-hidden border border-border/50">
      <div className="flex items-center justify-between px-3 py-1.5 border-b border-border/40" style={{ background: "hsl(var(--muted)/0.5)" }}>
        <span className="text-[10px] font-mono text-muted-foreground uppercase tracking-wider">{language}</span>
        <Button variant="ghost" size="icon" className="h-6 w-6 opacity-60 group-hover:opacity-100" onClick={handleCopy}>
          {copied ? <CheckCheck className="h-3 w-3 text-green-500"/> : <Copy className="h-3 w-3"/>}
        </Button>
      </div>
      <pre className="p-3 text-xs font-mono overflow-x-auto leading-relaxed whitespace-pre-wrap" style={{ background: "hsl(var(--muted)/0.25)", color: "hsl(var(--foreground))" }}>
        {code}
      </pre>
    </div>);
}
function Section({ title, children, defaultOpen = false, }) {
    ...
    return (<div className="border border-border/40 rounded-lg overflow-hidden">
      <button className="w-full flex items-center justify-between px-4 py-3 text-sm font-medium hover:bg-muted/20 transition-colors" onClick={() => setOpen((v) => !v)}>
        <span>{title}</span>
...
export default function PublicRegisterDocs() {
    return (<Card className="border-border/50">
      ...
      <CardContent className="space-y-4">
        ...
        ].map(([field, type, req, desc]) => (<tr key={field} className="border-b border-border/20 hover:bg-muted/10">
        ...
        <Section title="Error codes">
          <div className="overflow-x-auto rounded-lg border border-border/40">
            <table className="w-full text-xs">
              ...
              <tbody>
                {ERROR_TABLE.map(({ code, error, reason }) => (<tr key={error} className="border-b border-border/20 hover:bg-muted/10">
                    <td className="px-3 py-2">
                      <Badge variant="outline" className={`text-[10px] font-mono ${code.startsWith("2") ? "border-green-500/40 text-green-400" :
